...
def get_playlist_videos_count(title,playlist_sh):

    import json
    if playlist_sh == '':
        return 0
    commond = 'bash %s' % playlist_sh
    f = os.popen(commond,mode='r')
    s = f.read()
    f.close()
    res = json.loads(s)
    for l in res.get('playlists'):
        debug("%s : %s " % (l.get('title'), l.get('videosCount')))
        if l.get('title') == title:
            return l.get('videosCount')
    return 0
...
def upload_youtube_video(youtube, options, video_path, total_videos, index):
    ...
    if options.title_template:
        title_template = '{date_time} {video_count} {title}'
        video_count = get_playlist_videos_count(
            title, options.playlist_sh) + 1
        d = {'date_time': today, 'video_count': video_count, 'title': options.title}
        complete_title = title_template.format(**d)
    debug("video title : %s " % complete_title)
...
def main(arguments):
    ...
    parser.add_option('-t', '--title', dest='title', type="string",
                      help='Video title')
    parser.add_option('-c', '--category', dest='category', type="string", default='People & Blogs',
    ...
    parser.add_option('', '--title-template', dest='title_template',
                      ...
                      help='Use template for video ( {date_time} {video_count} {title} )')
    parser.add_option('-f', '--video_file', dest='video_file',
                      type='string', metavar='FILE', help='video file path')
    ...
    parser.add_option('', '--playlist-sh', dest='playlist_sh',default='',
                      type='string', help='studio.youtube.com Cookie and request Body curl')
